...
abstract class JSONElementStorage protected constructor(
    val fileSpec: String,
    ...
    final override fun getSerializedState(
        storageData: StateMap,
        component: Any?,
        componentName: String,
        archive: Boolean
    ) = storageData.getState(componentName, archive)

    final override fun archiveState(
        storageData: StateMap,
        componentName: String,
        serializedState: JSONObject?
    ) {
        storageData.archive(componentName, serializedState)
    }

    final override fun hasState(storageData: StateMap, componentName: String) =
        storageData.hasState(componentName)
                ...
                isLoadLocalData = !provider.read(fileSpec) { inputStream ->
                    inputStream?.let {
                        ...
                        providerDataStateChanged(
                            createDataWriterForElement(element!!, toString()),
    ...
    private fun loadState(element: JSONObject): StateMap {
        beforeElementLoaded(element)
        return StateMap.fromMap(FileStorageCoreUtil.load(element))
    }
    ...
    abstract class JSONElementStorageSaveSession<T : JSONElementStorage>(
        ...
        final override fun createSaveSession(): SaveSession? {
            ...
            val elements = save(
                stateMap,
                newLiveStates ?: throw IllegalStateException("createSaveSession was already called")
            )
            newLiveStates = null
        ...
        private inner class SaveExecutor(
            private val element: List<JSONObject>?,
        ...
        override fun setSerializedState(componentName: String, element: JSONObject?) {
            ...
            //val normalized = "root" //element?.normalizeRootName()
            if (copiedStates == null) {
                copiedStates =
                    setStateAndCloneIfNeeded(componentName, element, originalStates, newLiveStates)
            } else {
    ...
    protected open fun beforeElementSaved(
        elements: MutableList<JSONObject>,
        rootAttributes: MutableMap<String, String>
    ) {
    }
...
interface DataWriter {
    ...
    fun write(
        output: OutputStream,
        lineSeparator: String = LineSeparator.LF.separatorString,
        filter: DataWriterFilter? = null
    )

    fun hasData(filter: DataWriterFilter): Boolean
}

internal fun DataWriter?.writeTo(
    file: Path,
    lineSeparator: String = LineSeparator.LF.separatorString
) {
    if (this == null) {
...
internal class JsonDataWriter(private val data: List<JSONObject>) : StringDataWriter() {
    companion object {
        private val LOG = logger<JsonDataWriter>()
    }

    override fun hasData(filter: DataWriterFilter): Boolean {
        return data.any { filter.hasData(it) }
    }

    override fun write(writer: Writer, lineSeparator: String, filter: DataWriterFilter?) {
        val packJsonObject = JSONObject()
        val packJsonArray = JSONArray()
        data.forEach {
            packJsonArray.put(it)
        }

        packJsonObject.put(FileStorageCoreUtil.COMPONENT, packJsonArray)

        val formattedJson = packJsonObject.toString(1)
...
private fun StateMap.getChangedComponentNames(newStates: StateMap): Set<String> {
    ...
    val bothStates = ArrayList<String>(newKeys.size.coerceAtMost(existingKeys.size))
    val existingKeysSet =
        if (existingKeys.size < 3) existingKeys.asList() else CollectionFactory.createSmallMemoryFootprintSet(
            existingKeys.asList()
        )
    ...
    val diffs =
        CollectionFactory.createSmallMemoryFootprintSet<String>(newKeys.size + existingKeys.size)
...
internal fun createDataWriterForElement(
    element: JSONObject,
    storageFilePathForDebugPurposes: String
): DataWriter {
...
private fun save(
    states: StateMap,
    newLiveStates: Map<String, JSONObject>
    ...
    for (componentName in states.keys()) {
        ...
        try {
            element = states.getElement(componentName, newLiveStates)?.let {
                val copyNames = mutableListOf<String>()

                for (i in it.keys()) {
                    it.optString(i).let { copyNames.add(it) }
                }

                JSONObject(it, copyNames.toTypedArray())
            } ?: continue
        } catch (e: Exception) {
        ...
        val nameAttribute: String? = element.optString(FileStorageCoreUtil.NAME)

        @Suppress("SuspiciousEqualsCombination")
        if (nameAttribute != null && componentName == nameAttribute) {
            // all is OK
        } else {
            element.put(FileStorageCoreUtil.NAME, componentName)
        }
