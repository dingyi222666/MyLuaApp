...
interface MessageBus : Disposable {
    ...
    /**
     * Allows to create new connection that is bound to the given [Disposable]. That means that returned connection
     ...
     *
     *  1.
     * Messaging clients create new [connections][MessageBusConnection] within the target message bus and
     ...
     *
     *  1.
     * `Subscriber<sub>1</sub>` and `subscriber<sub>2</sub>` are registered for the same topic within
     ...
     *  1. Queued messages delivery ends as there are no messages queued but not dispatched;
