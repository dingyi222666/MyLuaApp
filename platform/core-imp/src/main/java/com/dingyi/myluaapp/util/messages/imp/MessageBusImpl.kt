...
open class MessageBusImpl : MessageBus {
    ...
    override fun <L : Any> syncPublisher(topic: Topic<L>): L {
        ...
        return publisherCache.computeIfAbsent(topic) { topic1: Topic<*> ->
            ...
            Proxy.newProxyInstance(
                aClass.classLoader,
                arrayOf(aClass),
                createPublisher(topic1, topic1.broadcastDirection)
            )
        } as L
    }

    internal open fun <L> createPublisher(
        topic: Topic<L>,
        direction: Topic.BroadcastDirection
    ): MessagePublisher<L> {
        return when (direction) {
            Topic.BroadcastDirection.TO_PARENT -> ToParentMessagePublisher(topic, this)
    ...
    fun notifyOnSubscription(topic: Topic<*>) {
        ...
        if (topic.broadcastDirection != Topic.BroadcastDirection.TO_CHILDREN) {
            return
        }
...
class RootBus(owner: MessageBusOwner) : CompositeMessageBus(owner) {
    ...
    private val queueThreadLocal: ThreadLocal<MessageQueue> =
        ThreadLocal.withInitial { MessageQueue() }
...
internal class MessageQueue {
    @JvmField
    val queue = java.util.ArrayDeque<Message>()
...
private fun computeMethodHandle(method: Method, args: Array<Any?>?): MethodHandle {
    method.setAccessible(true)
    val result: MethodHandle = try {
        LOOKUP.unreflect(method)
    } catch (e: IllegalAccessException) {
        throw java.lang.RuntimeException(e)
    }
    return if (args == null) result else result.asSpreader(Array<Any>::class.java, args.size)
}
...
internal fun executeOrAddToQueue(
    ...
    val methodHandle = computeMethodHandle(method, args)
    if (jobQueue == null) {
        var exceptions = prevExceptions
...
internal class ToParentMessagePublisher<L>(topic: Topic<L>, bus: MessageBusImpl) :
    ...
    override fun publish(method: Method, args: Array<Any?>?, queue: MessageQueue?): Boolean {
        ...
        while (true) {
            ...
            if (!handlers.isEmpty()) {
                hasHandlers = true
                exceptions =
                    executeOrAddToQueue(topic, method, args, handlers, queue, exceptions, bus)
            }
            parentBus = parentBus.parentBus ?: break
        }
...
private fun removeDisposedHandlers(
    topicAndHandlerPairs: Array<Any>,
    index: Int,
    topic: Topic<*>,
    bus: MessageBusImpl
...
private fun deliverImmediately(
    connection: MessageBusConnectionImpl,
    jobs: Deque<Message>
): List<Message>? {
    ...
    while (jobIterator.hasNext()) {
        ...
        while (i < length) {
            ...
            if (handler != null && connection.bus === job.bus && connection.isMyHandler(
                    job.topic,
                    handler
                )
            ) {
...
private fun invokeListener(
    ...
    try {
        ...
        if (handler is MessageHandler) {
            ...
            invokeMethod(handler, args, methodHandle)
            messageDeliveryListener.messageDelivered(
                topic,
                methodName,
                handler,
...
internal fun throwExceptions(exceptions: List<Throwable>) {
    ...
    exceptions.firstOrNull { it is ProcessCanceledException || it is CancellationException }
        ?.let { throw it }
