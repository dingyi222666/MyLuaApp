...
class ProjectFileHelper(private val mActivity: BaseActivity<*>, private val mCallBack: RefreshCallBack) {
    ...
    fun showFileSelectPopupMenu(filePath:String, showView: View,isHide:Boolean=false) {
        ...
        showView.post {
            getPrivateMethod(helper, "showPopup", arrayOf(Integer.TYPE, Integer.TYPE,
                    //xOffset yOffset
                    java.lang.Boolean.TYPE, java.lang.Boolean.TYPE), arrayOf(mActivity.dp2px(16), -showView.height, true, true))
        }
    }
    ...
    private fun showFileRenameDialog(path: String) {
        ...
        mInputDialog.setMode(InputDialog.Mode.INPUT_1)
                ...
                .setPositiveButton(android.R.string.ok) { _: DialogInterface?, _: Int ->
                    val renamePath = "$mParentPath/${mInputDialog.getBinding<DialogLayoutInput1Binding>().folder.text}"
    ...
    private fun showNewFileDialog(path: String, which: String) {
        ...
        mInputDialog.setMode(InputDialog.Mode.INPUT_1)
                ...
                .setPositiveButton(android.R.string.ok) { _: DialogInterface?, _: Int ->
                    val path = "$path/${mInputDialog.getBinding<DialogLayoutInput1Binding>().folder.text}${getFileTemplateSuffix(which)}"
